# End User License Agreement (EULA) — Organize Files

**Publisher:** Guțulov Răzvan Constantin PFA  
**Registered address:** Str. Republicii nr. 33B, bl. N3, sc. A, et. 1, ap. 3, Breaza de Sus, 105400 Breaza, jud. Prahova, România  
**Trade register:** F2026004513003 (EUID ROONRC.F2026004513003)  
**Tax identification number:** 53610310  
**Contact:** razvan.gutulov@outlook.com  
**Effective date:** 2026-05-28  
**Public URL (store listings):** `https://github.com/GutRaz/organize-files-legal/blob/main/en/eula.md`

---

## 1. Grant of license

Subject to payment (where required) and these terms, the publisher grants you a **non-exclusive, non-transferable** license to install and use Organize Files on devices you control, through the channel where you obtained it (Microsoft Store, Google Play, Mac App Store, Apple App Store (iOS), or a signed side-loaded build).

This license is granted **per installation**. A lifetime (one-time) purchase covers **one active installation at a time** — one computer or one device profile. Using Organize Files on an additional device requires an additional purchase for that device. A lifetime license may be moved to a replacement device when the earlier installation is no longer used. Copying license or profile files between devices does not grant access.

Subscriptions follow the store account on each enrolled device while the subscription is active.

Where the publisher operates a license server, a lifetime purchase binds to the first installation that registers it and further installations are refused. Where no license server is operated, this section still defines the permitted use.

## 2. Restrictions

You may not:

- Reverse engineer except where mandatory local law allows
- Remove copyright or license notices
- Redistribute the app or engine except as permitted by the store channel
- Use the app to violate third-party rights or applicable law

## 3. Backups and destructive operations

The app can **move or delete files** in any run where Dry run is off. **You** are responsible for verified backups before any real run.

## 4. Disclaimer of warranties

THE SOFTWARE IS PROVIDED **“AS IS”**, WITHOUT WARRANTY OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, AND NON-INFRINGEMENT, except where mandatory consumer law prohibits.

## 5. Limitation of liability

TO THE MAXIMUM EXTENT PERMITTED BY LAW, THE PUBLISHER SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES, OR FOR LOSS OF DATA, PROFITS, OR BUSINESS INTERRUPTION, ARISING FROM USE OR MISUSE OF THE SOFTWARE, except where mandatory consumer law prohibits.

## 6. Trial and subscriptions

Trial duration, store SKUs, and renewal terms are defined in the **store listing** and in-app license card. Release builds require valid store entitlement, an active trial, or publisher-operated license server authorization.

**After a store refund, revocation, or cancelled subscription:** if this installation previously recorded a paid store purchase, the app may start a **new 14-day local trial** (at most **two** such trials per installation). Money refunds are handled only by the store platform — not inside the app. See the in-app guide chapter *Refunds and trial after purchase* for the full flow.

## 7. Privacy

See [Privacy Policy](./privacy-policy.md).

## 8. EU/EEA right of withdrawal (14-day cooling-off period)

If you are a consumer resident in the **European Union or European Economic Area**, you have the right to withdraw from this purchase within **14 calendar days** of completing the transaction, without giving any reason (right of withdrawal under Directive 2011/83/EU).

**Waiver for digital content:** You lose the right of withdrawal only where you gave prior express consent for performance to begin during the 14-day period, acknowledged that giving that consent ends the right, and received confirmation of the contract on a durable medium, as Article 16(m) of Directive 2011/83/EU requires. The store you purchase through captures that consent and that acknowledgement at checkout.

For purchases made through Microsoft Store, Google Play, Mac App Store, or Apple App Store (iOS), use that platform's refund or withdrawal tools. If a store asks the publisher to review or process a request, the publisher handles it through that store's console according to platform policy and applicable law.

## 9. Termination

This license ends if you breach these terms or if the store revokes access. On termination, stop using the app and uninstall it.

## 10. Governing law

Unless mandatory consumer law in your country requires otherwise, disputes are governed by the laws of Romania, without regard to conflict-of-law rules.

Romanian consumers may contact the National Consumer Protection Authority (ANPC — Autoritatea Națională pentru Protecția Consumatorilor, www.anpc.ro) for disputes. EU/EEA consumers may use the competent national consumer authority or alternative dispute resolution entity available in their country; the former EU ODR platform has been discontinued since 20 July 2025.


## 11. Contact

**Guțulov Răzvan Constantin PFA** — razvan.gutulov@outlook.com
