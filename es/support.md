> **Versión en español.** En caso de conflicto, prevalece la versión en [inglés](../en/support.md).

---

# Soporte — Organize Files

¿Necesitas ayuda con **Organize Files**? Estaremos encantados de ayudarte.

## Contacto

- **Correo electrónico:** razvan.gutulov@outlook.com
- **Editor:** Guțulov Răzvan Constantin PFA
- **Domicilio registrado:** Str. Republicii nr. 33B, bl. N3, sc. A, et. 1, ap. 3, Breaza de Sus, 105400 Breaza, jud. Prahova, România
- **Registro mercantil:** F2026004513003 (EUID ROONRC.F2026004513003)
- **Número de identificación fiscal:** 53610310

Cuando nos escribas, incluye por favor:

- Tu sistema operativo (Windows, Linux, macOS, Android o iOS) y su versión
- La versión de la aplicación. El Asistente de guía de la aplicación la muestra cuando se le pregunta por la versión.
- La tienda desde la que instalaste (Microsoft Store, Google Play, Apple App Store o una compilación instalada manualmente)
- Una descripción clara del problema y los pasos para reproducirlo

Procuramos responder en **2–3 días laborables**.

## Antes de contactar con el soporte

- Antes de que una ejecución desde la ventana principal mueva o elimine archivos, la aplicación pide confirmación. **Simulación** muestra lo que ocurriría sin cambiar ningún archivo.
- Mantén siempre una **copia de seguridad verificada** antes de cualquier ejecución que mueva o elimine archivos.
- La documentación y el asistente de la aplicación responden muchas preguntas frecuentes, sin conexión.

## Facturación y reembolsos

Las compras y los reembolsos los gestiona la tienda donde compraste (Microsoft Store, Google Play o Apple App Store). El derecho de desistimiento de los consumidores en la Unión Europea y el Espacio Económico Europeo dura 14 días naturales desde la compra y se ejerce a través de la tienda. Escribir al correo de contacto no lo ejerce y no suspende ni amplía esos 14 días.

## Privacidad

Consulta la [Política de privacidad](./privacy-policy.md).

---

© 2026 Guțulov Răzvan Constantin PFA. Todos los derechos reservados.
