> **Suomenkielinen versio.** Ristiriitatilanteessa [englanninkielinen](../en/support.md) versio on määräävä.

---

# Tuki — Organize Files

Tarvitsetko apua **Organize Files** -sovelluksen kanssa? Autamme mielellämme.

## Yhteystiedot

- **Sähköposti:** razvan.gutulov@outlook.com
- **Julkaisija:** Guțulov Răzvan Constantin PFA
- **Rekisteröity osoite:** Str. Republicii nr. 33B, bl. N3, sc. A, et. 1, ap. 3, Breaza de Sus, 105400 Breaza, jud. Prahova, România
- **Kaupparekisteri:** F2026004513003 (EUID ROONRC.F2026004513003)
- **Verotunniste:** 53610310

Kun kirjoitat meille, liitä mukaan:

- Käyttöjärjestelmäsi (Windows, Linux, macOS, Android tai iOS) ja sen versio
- Sovelluksen versio. Sovelluksen Opas-avustaja näyttää sen, kun siltä kysytään versiota.
- Kauppa, josta asensit (Microsoft Store, Google Play, Apple App Store tai manuaalisesti asennettu koontiversio)
- Selkeä kuvaus ongelmasta ja vaiheet sen toistamiseksi

Pyrimme vastaamaan **2–3 arkipäivän** kuluessa. Hyvityksiä koskevat kysymykset voivat kestää jopa **14 päivää**.

## Ennen kuin otat yhteyttä tukeen

- Ennen kuin pääikkunan ajo siirtää tai poistaa tiedostoja, sovellus pyytää vahvistuksen. **Testiajo** näyttää, mitä tapahtuisi, muuttamatta yhtään tiedostoa.
- Pidä aina **varmennettu varmuuskopio** ennen suoritusta, joka siirtää tai poistaa tiedostoja.
- Sovelluksen dokumentaatio ja avustaja vastaavat moniin yleisiin kysymyksiin offline-tilassa.

## Laskutus ja hyvitykset

Ostokset ja hyvitykset hoitaa kauppa, josta ostit (Microsoft Store, Google Play tai Apple App Store).

## Tietosuoja

Katso [Tietosuojakäytäntö](./privacy-policy.md).

---

© 2026 Guțulov Răzvan Constantin PFA. Kaikki oikeudet pidätetään.
