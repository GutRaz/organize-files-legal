> **Traducere pusă la dispoziție pentru comoditate.** Versiunile de referință sunt [EULA în engleză](../en/eula.md) și [Politica de confidențialitate în engleză](../en/privacy-policy.md). Acolo unde legea imperativă de protecție a consumatorilor din țara dumneavoastră dă prioritate versiunii în limba proprie, se aplică acea versiune. Nu este sfat juridic — consultați un avocat calificat în jurisdicția dumneavoastră.

---

# Acord de licență pentru utilizatorul final (EULA) — Organize Files

**Editor:** Guțulov Răzvan Constantin PFA  
**Sediu profesional:** Str. Republicii nr. 33B, bl. N3, sc. A, et. 1, ap. 3, Breaza de Sus, 105400 Breaza, jud. Prahova, România  
**Registrul comerțului:** F2026004513003 (EUID ROONRC.F2026004513003)  
**Cod unic de înregistrare:** 53610310  
**Contact:** razvan.gutulov@outlook.com  
**Data efectivă:** 2026-05-28  

## 1. Acordarea licenței

Sub rezerva plății (unde este cazul) și a acestor termeni, editorul acordă o licență **neexclusivă, netransferabilă** de instalare și utilizare a Organize Files pe dispozitivele controlate, prin canalul de achiziție (Microsoft Store, Google Play, Mac App Store, Apple App Store (iOS) sau build semnat).

Licența se acordă **per instalare**. O achiziție lifetime (o singură plată) acoperă **o singură instalare activă** — un computer sau un profil de dispozitiv. Utilizarea Organize Files pe un dispozitiv suplimentar necesită o achiziție suplimentară pentru acel dispozitiv. O licență lifetime poate fi mutată pe un dispozitiv de înlocuire când instalarea anterioară nu mai este folosită. Copierea fișierelor de licență sau de profil între dispozitive nu acordă acces.

Abonamentele urmează contul de magazin pe fiecare dispozitiv înrolat, cât timp abonamentul este activ.

Când editorul operează un server de licențe, o achiziție lifetime se leagă de prima instalare care o înregistrează, iar instalările ulterioare sunt refuzate. Când nu se operează niciun server de licențe, această secțiune definește în continuare utilizarea permisă.

## 2. Restricții

Nu este permis: inginerie inversă (exceptând unde legea o impune), eliminarea mențiunilor de copyright, redistribuirea neautorizată, utilizarea ilegală.

## 3. Copii de siguranță și operații distructive

Aplicația poate **muta sau șterge fișiere** în orice rulare în care Simularea este oprită. **Utilizatorul** este responsabil pentru copii de siguranță verificate.

## 4. Exonerare de garanții

SOFTWARE-UL ESTE FURNIZAT **„CA ATARE”**, FĂRĂ NICIO GARANȚIE DE ORICE FEL, EXPRESĂ SAU IMPLICITĂ, INCLUSIV GARANȚIILE DE VANDABILITATE, DE ADECVARE PENTRU UN ANUMIT SCOP ȘI DE NEÎNCĂLCARE A DREPTURILOR TERȚILOR, cu excepția cazurilor în care legea imperativă de protecție a consumatorilor interzice.

## 5. Limitarea răspunderii

ÎN LIMITA MAXIMĂ PERMISĂ DE LEGE, EDITORUL NU RĂSPUNDE PENTRU NICIUN PREJUDICIU INDIRECT, INCIDENTAL, SPECIAL, SUBSECVENT, CONSECVENȚIAL SAU PUNITIV ȘI NICI PENTRU PIERDEREA DE DATE, PIERDEREA DE PROFIT (BENEFICIUL NEREALIZAT) SAU ÎNTRERUPEREA ACTIVITĂȚII, REZULTATE DIN UTILIZAREA SAU UTILIZAREA NECORESPUNZĂTOARE A SOFTWARE-ULUI, cu excepția cazurilor în care legea imperativă de protecție a consumatorilor interzice.

## 6. Probă și abonamente

Durata probei, SKU-urile și reînnoirea sunt definite în listarea magazinului și în cardul de licență din aplicație.

**După refund, revocare sau anularea abonamentului în magazin:** dacă această instalare a înregistrat anterior o achiziție plătită, aplicația poate porni o **probă locală nouă de 14 zile** (cel mult **două** astfel de probe per instalare). Rambursările în bani sunt gestionate doar de platforma magazinului — nu în aplicație. Vezi în ghidul din aplicație capitolul *Rambursare și trial după achiziție*.

## 7. Confidențialitate

Vezi [Politica de confidențialitate](./privacy-policy.md).

## 8. Dreptul de retragere al consumatorilor din UE/SEE (perioadă de 14 zile)

Dacă ești un consumator rezident în **Uniunea Europeană sau Spațiul Economic European**, ai dreptul să te retragi din această achiziție în termen de **14 zile calendaristice** de la finalizarea tranzacției, fără a furniza niciun motiv (drept de retragere conform Directivei 2011/83/UE).

**Renunțarea la drept pentru conținut digital:** Pierzi dreptul de retragere doar dacă ai dat un acord prealabil expres pentru începerea executării în perioada de 14 zile, ai confirmat că acest acord stinge dreptul și ai primit confirmarea contractului pe un suport durabil, așa cum cere articolul 16(m) din Directiva 2011/83/UE. Magazinul prin care cumperi înregistrează acest acord și această confirmare la finalizarea comenzii.

Pentru achiziții făcute prin Microsoft Store, Google Play, Mac App Store sau Apple App Store (iOS), folosește instrumentele de rambursare sau retragere ale platformei. Dacă magazinul cere editorului să analizeze sau să proceseze o cerere, editorul o tratează în consola acelui magazin conform politicii platformei și legii aplicabile.

## 9. Încetare

Licența încetează la încălcarea termenilor sau revocarea accesului de către magazin.

## 10. Lege aplicabilă

Exceptând legea imperativă de protecție a consumatorilor, litigiile sunt guvernate de legile României.

Consumatorii din România se pot adresa Autorității Naționale pentru Protecția Consumatorilor (ANPC, www.anpc.ro) pentru litigii. Consumatorii din UE/SEE pot apela la autoritatea națională competentă pentru protecția consumatorilor sau la o entitate de soluționare alternativă a litigiilor din țara lor. Fosta platformă SOL a UE nu mai funcționează din 20 iulie 2025.


## 11. Contact

**Guțulov Răzvan Constantin PFA** — razvan.gutulov@outlook.com

## Documente conexe

- [EULA (engleză)](../en/eula.md)  
- [Politica de confidențialitate (engleză)](../en/privacy-policy.md)
