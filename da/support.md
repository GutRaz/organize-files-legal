> **Dansk version.** I tilfælde af konflikt har den [engelske](../en/support.md) version forrang.

---

# Support — Organize Files

Har du brug for hjælp med **Organize Files**? Vi hjælper dig gerne.

## Kontakt

- **E-mail:** razvan.gutulov@outlook.com
- **Udgiver:** Guțulov Răzvan Constantin PFA
- **Registreret adresse:** Str. Republicii nr. 33B, bl. N3, sc. A, et. 1, ap. 3, Breaza de Sus, 105400 Breaza, jud. Prahova, România
- **Handelsregister:** F2026004513003 (EUID ROONRC.F2026004513003)
- **Skatteregistreringsnummer:** 53610310

Når du skriver til os, bedes du oplyse:

- Dit operativsystem (Windows, Linux, macOS, Android eller iOS) og dets version
- App-versionen. Guide assistent i appen viser den, når man spørger om versionen.
- Den butik, du installerede fra (Microsoft Store, Google Play, Apple App Store eller en manuelt installeret build)
- En klar beskrivelse af problemet og trinnene til at genskabe det

Vi bestræber os på at svare inden for **2-3 hverdage**.

## Før du kontakter supporten

- Før en kørsel i hovedvinduet flytter eller sletter filer, beder appen om bekræftelse. **Prøvekørsel** viser, hvad der ville ske, uden at ændre nogen fil.
- Hav altid en **verificeret sikkerhedskopi**, før du kører en handling, der flytter eller sletter filer.
- Dokumentationen og assistenten i appen besvarer mange almindelige spørgsmål offline.

## Fakturering og refusioner

Køb og refusioner håndteres af den butik, hvor du købte (Microsoft Store, Google Play eller Apple App Store). Fortrydelsesretten for forbrugere i Den Europæiske Union og Det Europæiske Økonomiske Samarbejdsområde gælder i 14 kalenderdage fra købet og udøves gennem butikken. En e-mail til kontaktadressen udøver ikke retten og hverken stopper eller forlænger de 14 dage.

## Privatliv

Se [Privatlivspolitikken](./privacy-policy.md).

---

© 2026 Guțulov Răzvan Constantin PFA. Alle rettigheder forbeholdes.
