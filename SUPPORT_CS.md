> **Česká verze.** V případě rozporu je rozhodující [anglická](./SUPPORT.md) verze.

---

# Podpora — Organize Files

Potřebujete pomoc s aplikací **Organize Files**? Rádi vám pomůžeme.

## Kontakt

- **E-mail:** razvan.gutulov@outlook.com
- **Vydavatel:** Guțulov Răzvan Constantin PFA
- **Sídlo:** Str. Republicii nr. 33B, bl. N3, sc. A, et. 1, ap. 3, Breaza de Sus, 105400 Breaza, jud. Prahova, România
- **Obchodní rejstřík:** F2026004513003 (EUID ROONRC.F2026004513003)
- **Daňové identifikační číslo:** 53610310

Když nám napíšete, uveďte prosím:

- Váš operační systém (Windows, Linux, macOS, Android nebo iOS) a jeho verzi
- Verzi aplikace. Asistent dokumentace v aplikaci ji ukáže, když se ho zeptáte na verzi.
- Obchod, ze kterého jste instalovali (Microsoft Store, Google Play, Apple App Store nebo ručně nainstalované sestavení)
- Jasný popis problému a kroky k jeho reprodukci

Snažíme se odpovědět do **2–3 pracovních dnů**. Dotazy na vrácení peněz mohou trvat až **14 dní**.

## Než kontaktujete podporu

- Než spuštění z hlavního okna přesune nebo smaže soubory, aplikace si vyžádá potvrzení. **Zkušební běh** ukáže, co by se stalo, aniž by změnil jakýkoli soubor.
- Před každým spuštěním, které přesouvá nebo maže soubory, si vždy uchovejte **ověřenou zálohu**.
- Dokumentace a asistent v aplikaci odpovídají na mnoho častých otázek offline.

## Fakturace a vrácení peněz

Nákupy a vrácení peněz zajišťuje obchod, ve kterém jste nakoupili (Microsoft Store, Google Play nebo Apple App Store).

## Ochrana soukromí

Viz [Zásady ochrany osobních údajů](./PRIVACY_POLICY_CS.md).

---

© 2026 Guțulov Răzvan Constantin PFA. Všechna práva vyhrazena.
