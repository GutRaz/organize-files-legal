> **A fordítást a könnyebb tájékozódás érdekében adjuk közre.** A referenciaváltozatok az [angol EULA](./EULA_EN.md) és az [angol adatvédelmi szabályzat](./PRIVACY_POLICY_EN.md). Ahol az Ön országának kógens fogyasztóvédelmi joga elsőbbséget ad az Ön saját nyelvén készült változatnak, az a változat irányadó. Ez nem jogi tanács — forduljon szakképzett ügyvédhez a saját joghatóságában.

---

# Végfelhasználói licencszerződés (EULA) — Organize Files

**Kiadó:** Guțulov Răzvan Constantin PFA  
**Bejegyzett cím:** Str. Republicii nr. 33B, bl. N3, sc. A, et. 1, ap. 3, Breaza de Sus, 105400 Breaza, jud. Prahova, România  
**Cégjegyzék:** F2026004513003 (EUID ROONRC.F2026004513003)  
**Adóazonosító szám:** 53610310  
**Kapcsolat:** razvan.gutulov@outlook.com  
**Hatálybalépés dátuma:** 2026-05-28  
**Nyilvános URL (üzleti adatok):** `https://github.com/GutRaz/organize-files-legal/blob/main/EULA_HU.md`

---

## 1. Engedély megadása

Fizetés (ahol szükséges) és a jelen feltételek függvényében a kiadó **nem kizárólagos, nem átruházható** licencet ad Önnek az Organe Files telepítésére és használatára az Ön által ellenőrzött eszközökön, azon a csatornán keresztül, ahol azt megszerezte (Microsoft Store, Google Play, Mac App Store, Apple App Store (iOS) vagy aláírt, oldalról betöltött build).

A licencet **telepítésenként** adjuk. Az élettartamra szóló (egyszeri) vásárlás **egyszerre egy aktív telepítésre** vonatkozik — egy számítógépre vagy egy eszközprofilra. Az Organize Files további eszközön való használatához az adott eszközre külön vásárlás szükséges. Az élettartamra szóló licenc áthelyezhető csereeszközre, ha a korábbi telepítést már nem használják. A licenc- vagy profilfájlok eszközök közötti másolása nem biztosít hozzáférést.

Az előfizetések az áruházfiókot követik minden regisztrált eszközön, amíg az előfizetés aktív.

Ha a kiadó licencszervert üzemeltet, az élettartamra szóló vásárlás az első regisztráló telepítéshez kötődik, a további telepítéseket pedig elutasítja. Ha nem üzemeltet licencszervert, ez a szakasz továbbra is meghatározza a megengedett használatot.

## 2. Korlátozások

Nem lehet:

- Visszafejtés, kivéve, ha a kötelező helyi törvények megengedik
- Távolítsa el a szerzői jogokra vagy licencekre vonatkozó megjegyzéseket
- Az alkalmazás vagy a motor újraterjesztése, kivéve az áruházi csatorna által engedélyezett eseteket
- Használja az alkalmazást harmadik felek jogainak vagy a vonatkozó jogszabályok megsértésére

## 3. Biztonsági mentések és romboló műveletek

Az alkalmazás **mozgathat vagy törölhet fájlokat** minden olyan futtatásban, amelyben a Próbafuttatás ki van kapcsolva. **Ön** felelős az ellenőrzött biztonsági másolatokért, mielőtt bármilyen futtatást végeznének.

## 4. A jótállás kizárása

A SZOFTVER **„AHOGY VAN”**, BÁRMILYEN KIFEJEZETT VAGY VÉLEMEZTETETT GARANCIA NÉLKÜL ÁLLÍTOTT, BELEÉRTVE AZ ELADHATÓSÁGOT, A MEGHATÁROZOTT CÉLRA VALÓ ALKALMASSÁGOT ÉS NEM JOGSÉRTÉST, kivéve, ahol a kötelező fogyasztóvédelmi jog ezt tiltja.

## 5. A felelősség korlátozása

A TÖRVÉNY ÁLTAL ENGEDÉLYEZETT MAXIMÁLIS MÉRTÉKÉBEN A KIADÓ NEM VÁLLAL FELELŐSSÉGET SEMMILYEN KÖZVETETT, VÉLETLENES, KÜLÖNLEGES, KÖVETKEZMÉNYES VAGY BÜNTETŐ KÁROKÉRT, VAGY ADATKEZÉSÉRT, AZ EREDMÉNY, VAGY ÜZLETI FELHASZNÁLÁSBÓL, VAGY VÁLLALKOZÁSBAN. SZOFTVER, kivéve, ahol a kötelező fogyasztóvédelmi jog ezt tiltja.

## 6. Próbaverzió és előfizetések

A próbaidőszak, az áruházi cikkszámok és a megújítási feltételek az **áruház adatlapján** és az alkalmazáson belüli licenckártyán vannak meghatározva. A kiadási buildekhez érvényes jogosultság szükséges, kivéve, ha a dokumentált fejlesztői/tesztelési felülírás aktív.

**Az üzletben történt visszatérítés, visszavonás vagy lemondott előfizetés után:** ha ez a telepítés korábban fizetős bolti vásárlást rögzített, az alkalmazás elindíthat egy **új 14 napos helyi próbaverziót** (telepítésenként legfeljebb **két** ilyen próba). A pénzvisszatérítést csak az áruház platformja kezeli – az alkalmazáson belül nem. Tekintse meg az alkalmazáson belüli útmutató *Visszatérítések és vásárlás utáni próbaverzió* című fejezetét a teljes folyamatért.

## 7. Adatvédelem

Lásd: [Adatvédelmi szabályzat](./PRIVACY_POLICY_HU.md).

## 8. EU/EGT elállási jog (14 napos elállási idő)

Ha Ön az **Európai Unióban vagy az Európai Gazdasági Térségben** lakóhellyel rendelkező fogyasztó, jogában áll a tranzakció teljesítését követő **14 naptári napon** indoklás nélkül elállni ettől a vásárlástól (2011/83/EU irányelv szerinti elállási jog).

**Lemondás a digitális tartalomról:** Az elállási jogot csak akkor veszíti el, ha előzetesen kifejezetten hozzájárult ahhoz, hogy a teljesítés a 14 napos határidőn belül megkezdődjön, tudomásul vette, hogy e hozzájárulással a jog megszűnik, és tartós adathordozón megkapta a szerződés visszaigazolását, ahogyan azt a 2011/83/EU irányelv 16. cikk m) pontja megköveteli. Az áruház, amelyen keresztül vásárol, a fizetéskor rögzíti ezt a hozzájárulást és tudomásulvételt.

A Microsoft Store, Google Play, Mac App Store vagy Apple App Store (iOS) használatával történő vásárlások esetén használja az adott platform visszatérítési vagy visszavonási eszközeit. Ha egy bolt egy kérés felülvizsgálatára vagy feldolgozására kéri a kiadót, a kiadó az adott áruház konzolján keresztül kezeli azt a platformszabályzatnak és a vonatkozó jogszabályoknak megfelelően.

## 9. Felmondás

Ez a licenc megszűnik, ha megszegi ezeket a feltételeket, vagy ha az áruház visszavonja a hozzáférést. Felmondáskor hagyja abba az alkalmazás használatát, és távolítsa el.

## 10. Irányadó jog

Hacsak az Ön országában a kötelező fogyasztóvédelmi jog másként nem rendelkezik, a vitákra Románia törvényei az irányadók, a kollíziós szabályok figyelembevétele nélkül.

A román fogyasztók vitáikkal az Országos Fogyasztóvédelmi Hatósághoz (ANPC – Autoritatea Națională pentru Protecția Consumatorilor, www.anpc.ro) fordulhatnak. Az EU/EGT fogyasztói használhatják az országukban elérhető illetékes nemzeti fogyasztóvédelmi hatóságot vagy alternatív vitarendezési szervezetet; a korábbi EU ODR platform 2025. július 20. óta megszűnt.


## 11. Kapcsolat

**Guțulov Răzvan Constantin PFA** — razvan.gutulov@outlook.com
