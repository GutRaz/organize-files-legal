> **Magyar változat.** Eltérés esetén az [angol](../en/support.md) változat az irányadó.

---

# Támogatás — Organize Files

Segítségre van szükséged az **Organize Files** használatához? Szívesen segítünk.

## Kapcsolat

- **E-mail:** razvan.gutulov@outlook.com
- **Kiadó:** Guțulov Răzvan Constantin PFA
- **Bejegyzett cím:** Str. Republicii nr. 33B, bl. N3, sc. A, et. 1, ap. 3, Breaza de Sus, 105400 Breaza, jud. Prahova, România
- **Cégjegyzék:** F2026004513003 (EUID ROONRC.F2026004513003)
- **Adóazonosító szám:** 53610310

Amikor írsz nekünk, kérjük, add meg:

- Az operációs rendszeredet (Windows, Linux, macOS, Android vagy iOS) és annak verzióját
- Az alkalmazás verzióját. Az alkalmazás Útmutató asszisztense megmutatja, ha a verzióról kérdezik.
- Az áruházat, ahonnan telepítetted (Microsoft Store, Google Play, Apple App Store vagy manuálisan telepített build)
- A probléma világos leírását és a reprodukálás lépéseit

Igyekszünk **2–3 munkanapon** belül válaszolni. A visszatérítéssel kapcsolatos kérdések akár **14 napot** is igénybe vehetnek.

## Mielőtt kapcsolatba lépsz a támogatással

- Mielőtt a főablakból indított futtatás fájlokat helyezne át vagy törölne, az alkalmazás megerősítést kér. A **Próbafuttatás** megmutatja, mi történne, anélkül hogy bármely fájlt megváltoztatna.
- Mindig tarts **ellenőrzött biztonsági másolatot**, mielőtt olyan futtatást indítasz, amely fájlokat helyez át vagy töröl.
- Az alkalmazás dokumentációja és asszisztense sok gyakori kérdésre offline is választ ad.

## Számlázás és visszatérítés

A vásárlásokat és a visszatérítéseket az az áruház kezeli, ahol vásároltál (Microsoft Store, Google Play vagy Apple App Store).

## Adatvédelem

Lásd az [Adatvédelmi szabályzatot](./privacy-policy.md).

---

© 2026 Guțulov Răzvan Constantin PFA. Minden jog fenntartva.
