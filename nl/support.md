> **Nederlandse versie.** Bij tegenstrijdigheid prevaleert de [Engelse](../en/support.md) versie.

---

# Ondersteuning — Organize Files

Hulp nodig bij **Organize Files**? We helpen u graag.

## Contact

- **E-mail:** razvan.gutulov@outlook.com
- **Uitgever:** Guțulov Răzvan Constantin PFA
- **Geregistreerd adres:** Str. Republicii nr. 33B, bl. N3, sc. A, et. 1, ap. 3, Breaza de Sus, 105400 Breaza, jud. Prahova, România
- **Handelsregister:** F2026004513003 (EUID ROONRC.F2026004513003)
- **Fiscaal identificatienummer:** 53610310

Vermeld bij uw bericht:

- Uw besturingssysteem (Windows, Linux, macOS, Android of iOS) en de versie ervan
- De app-versie. De Gids assistent in de app toont die wanneer naar de versie wordt gevraagd.
- De store waaruit u hebt geïnstalleerd (Microsoft Store, Google Play, Apple App Store of een handmatig geïnstalleerde build)
- Een duidelijke beschrijving van het probleem en de stappen om het te reproduceren

We streven ernaar binnen **2–3 werkdagen** te reageren.

## Voordat u contact opneemt met support

- Voordat een uitvoering in het hoofdvenster bestanden verplaatst of verwijdert, vraagt de app om bevestiging. **Proefrun** toont wat er zou gebeuren zonder een bestand te wijzigen.
- Bewaar altijd een **geverifieerde back-up** vóór een uitvoering die bestanden verplaatst of verwijdert.
- De documentatie en de assistent in de app beantwoorden veel voorkomende vragen offline.

## Facturering en terugbetalingen

Aankopen en terugbetalingen worden afgehandeld door de store waar u hebt gekocht (Microsoft Store, Google Play of Apple App Store). Het herroepingsrecht voor consumenten in de Europese Unie en de Europese Economische Ruimte geldt 14 kalenderdagen vanaf de aankoop en wordt uitgeoefend via de winkel. Een e-mail naar het contactadres oefent het niet uit en schort die 14 dagen niet op en verlengt ze ook niet.

## Privacy

Zie het [Privacybeleid](./privacy-policy.md).

---

© 2026 Guțulov Răzvan Constantin PFA. Alle rechten voorbehouden.
