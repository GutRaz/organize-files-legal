> **Versão em português.** Em caso de conflito, prevalece a versão em [inglês](../en/support.md).

---

# Suporte — Organize Files

Precisa de ajuda com o **Organize Files**? Teremos todo o gosto em ajudar.

## Contacto

- **E-mail:** razvan.gutulov@outlook.com
- **Editor:** Guțulov Răzvan Constantin PFA
- **Endereço registado:** Str. Republicii nr. 33B, bl. N3, sc. A, et. 1, ap. 3, Breaza de Sus, 105400 Breaza, jud. Prahova, România
- **Registo comercial:** F2026004513003 (EUID ROONRC.F2026004513003)
- **Número de identificação fiscal:** 53610310

Quando nos escrever, inclua por favor:

- O seu sistema operativo (Windows, Linux, macOS, Android ou iOS) e a respetiva versão
- A versão da aplicação. O Assistente de guia na aplicação mostra-a quando lhe perguntam pela versão.
- A loja a partir da qual instalou (Microsoft Store, Google Play, Apple App Store ou uma compilação instalada manualmente)
- Uma descrição clara do problema e os passos para o reproduzir

Procuramos responder em **2 a 3 dias úteis**. As questões de reembolso podem demorar até **14 dias**.

## Antes de contactar o suporte

- Antes de uma execução na janela principal mover ou eliminar ficheiros, a aplicação pede confirmação. **Simulação** mostra o que aconteceria sem alterar nenhum ficheiro.
- Mantenha sempre uma **cópia de segurança verificada** antes de qualquer execução que mova ou elimine ficheiros.
- A documentação e o assistente na aplicação respondem a muitas perguntas frequentes, offline.

## Faturação e reembolsos

As compras e os reembolsos são geridos pela loja onde comprou (Microsoft Store, Google Play ou Apple App Store).

## Privacidade

Consulte a [Política de Privacidade](./privacy-policy.md).

---

© 2026 Guțulov Răzvan Constantin PFA. Todos os direitos reservados.
