# Support — Organize Files

Need help with **Organize Files**? We are happy to assist.

## Contact

- **Email:** razvan.gutulov@outlook.com
- **Publisher:** Guțulov Răzvan Constantin PFA
- **Registered address:** Str. Republicii nr. 33B, bl. N3, sc. A, et. 1, ap. 3, Breaza de Sus, 105400 Breaza, jud. Prahova, România
- **Trade register:** F2026004513003 (EUID ROONRC.F2026004513003)
- **Tax identification number:** 53610310

When you write, please include:

- Your operating system (Windows, Linux, macOS, Android, or iOS) and its version
- The app version. The Guide assistant in the app shows it when asked about the version.
- The store you installed from (Microsoft Store, Google Play, Apple App Store, or a side-loaded build)
- A clear description of the problem and the steps to reproduce it

We aim to reply within **2–3 business days**. Refund questions can take up to **14 days**.

## Before contacting support

- Before a run in the main window moves or deletes files, the app asks for confirmation. **Dry run** shows what would happen without changing any file.
- Always keep a **verified backup** before any real run that moves or deletes files.
- The in-app documentation and assistant answer many common questions offline.

## Billing and refunds

Purchases and refunds are handled by the store you bought from (Microsoft Store, Google Play, or Apple App Store).

## Privacy

See the [Privacy Policy](./PRIVACY_POLICY_EN.md).

---

© 2026 Guțulov Răzvan Constantin PFA. All rights reserved.
