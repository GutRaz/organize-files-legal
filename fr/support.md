> **Version française.** En cas de conflit, la version [anglaise](../en/support.md) prévaut.

---

# Assistance — Organize Files

Besoin d'aide avec **Organize Files** ? Nous sommes ravis de vous aider.

## Contact

- **E-mail :** razvan.gutulov@outlook.com
- **Éditeur :** Guțulov Răzvan Constantin PFA
- **Adresse enregistrée:** Str. Republicii nr. 33B, bl. N3, sc. A, et. 1, ap. 3, Breaza de Sus, 105400 Breaza, jud. Prahova, România
- **Registre du commerce:** F2026004513003 (EUID ROONRC.F2026004513003)
- **Numéro d'identification fiscale:** 53610310

Lorsque vous nous écrivez, veuillez indiquer :

- Votre système d'exploitation (Windows, Linux, macOS, Android ou iOS) et sa version
- La version de l'application. L'Assistant-guide de l'application l'indique quand on lui demande la version.
- Le store depuis lequel vous avez installé (Microsoft Store, Google Play, Apple App Store ou un build installé manuellement)
- Une description claire du problème et les étapes pour le reproduire

Nous nous efforçons de répondre sous **2 à 3 jours ouvrés**. Les questions de remboursement peuvent prendre jusqu'à **14 jours**.

## Avant de contacter l'assistance

- Avant qu'une exécution lancée depuis la fenêtre principale ne déplace ou ne supprime des fichiers, l'application demande une confirmation. **Simulation** montre ce qui se passerait sans modifier aucun fichier.
- Conservez toujours une **sauvegarde vérifiée** avant toute exécution qui déplace ou supprime des fichiers.
- La documentation et l'assistant intégrés répondent à de nombreuses questions courantes, hors ligne.

## Facturation et remboursements

Les achats et les remboursements sont gérés par le store où vous avez effectué l'achat (Microsoft Store, Google Play ou Apple App Store).

## Confidentialité

Consultez la [Politique de confidentialité](./privacy-policy.md).

---

© 2026 Guțulov Răzvan Constantin PFA. Tous droits réservés.
